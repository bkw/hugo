...
LightState.prototype.set = function (keyVal, cb) {
    var lightstate = this,
        newValues = {},
        transition = keyVal.transitiontime;

    Object.keys(keyVal).forEach(function (k) {
        if (k === 'transitiontime') {
            return;
        }
        if (lightstate.state[k] != keyVal[k]) {
            newValues[k] = keyVal[k];
        }
    });
    if (! Object.keys(newValues).length) {
        console.error('no new values: (new/old)', keyVal, this.state);
        if (cb) {
            cb(null, newValues);
        }
        return;
    ...
    if ('undefined' !== typeof(transition)) {
        newValues.transitiontime = transition;
    }
    console.log('New Values', newValues, this.state);
    this.bridge.put(
